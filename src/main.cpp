...
void printBenchmarksJSON(int signal) {
    std::cout << std::endl;
    sharp::Benchmark::printBenchmarksJSON(std::cout, true);
    _exit(signal);
}
...
int main(int argc, char *argv[]) {
    ...
    //TODO: change to proper output class and interfaces
    if (opts.printBenchmarks){
        if (opts.printJSON){
            signal(SIGINT, printBenchmarksJSON);
            signal(SIGTERM, printBenchmarksJSON);
        }
        else if (opts.printMachineReadable) {
    ...
    std::cerr << "Initializing solver..." << std::endl;
